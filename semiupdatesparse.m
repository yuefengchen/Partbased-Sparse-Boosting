function  semiupdatesparse( sumimagedata, patch, label, importance, mostselected)
    global parameter;
    global haarfeature;
    global selectors;
    global alpha;
    ...
    for selno = 1:size(mostselected, 1)
        i = mostselected(selno, 1);
        ...
        selectclassifierindex = mostselected(selno, 2);
        ...
        selectors(i) = mostselected(selno, 2);
        if minerror >= 0.5
            alpha(i) = 0;
        else
    ...
    selectid = 1:parameter.numselectors;
    selectid(mostselected(:,1)) = [];
    for selno = 1:length(selectid)
