#ifndef MILSPECIAL_CLASSIFIER_MATLAB_HEADER
#define MILSPECIAL_CLASSIFIER_MATLAB_HEADER
/* ==========================================================================
 * classifier.h
 * parameter0  pathces
 * parameter1  alpha
                ...
                )
{
    ...
    // add for mil boosting  for calculate p(i);
    double * pos_sigma = new double [numofselectors];
    double * neg_sigma = new double [numofselectors];
    
    
    double posv, poss, negv, negs, posibility_pos, posibility_neg, h_patch;
    ...
    for(int iselector = 0; iselector < numofselectors; ++ iselector){
        locationIndexInt[iselector] = (int)haarfeature[0][INDEX].data[selectorInt[iselector]] - 1;
        #ifdef _DEBUG_
    ...
    for(int iselector = 0; iselector < numofselectors; ++ iselector) {
        location[iselector] = haarfeature[0][LOCATION].data + locationIndexInt[iselector];
        weight[iselector] = haarfeature[0][WEIGHT].data + locationIndexInt[iselector];
    }
        
    for(int iselector = 0; iselector < numofselectors; ++ iselector) {
        areanum[iselector] = (int)haarfeature[0][AREA].data[selectorInt[iselector]];
    }
    for(int iselector = 0; iselector < numofselectors; ++ iselector) {
        locationM[iselector] = haarfeature[0][LOCATION].M;
    }
    for(int iselector = 0; iselector < numofselectors; ++ iselector) {
        pos_mean[iselector] = haarfeature[0][POSGAUSSIAN].data[selectorInt[iselector]];
        pos_sigma[iselector] = haarfeature[0][POSGAUSSIAN].data[selectorInt[iselector] + featureNumInEachSelector];
        
        neg_mean[iselector] = haarfeature[0][NEGGAUSSIAN].data[selectorInt[iselector]];
        neg_sigma[iselector] = haarfeature[0][NEGGAUSSIAN].data[selectorInt[iselector] + featureNumInEachSelector];
        #ifdef _DEBUG_
    ...
    for(int kpatch = 0; kpatch < npatches; ++kpatch ) {
        ...
        for(int iselector = 0; iselector < numofselectors; ++ iselector){
            ...
            */
            ...
            posv = pos_mean[iselector], negv = neg_mean[iselector];
            poss = pos_sigma[iselector], negs = neg_sigma[iselector];
            posibility_pos = exp( (weightsum - posv)*(weightsum - posv)/(2*poss * poss))/poss + 1e-5;
            posibility_neg = exp( (weightsum - negv)*(weightsum - negv)/(2*negs * negs))/negs + 1e-5;
            h_patch += log(posibility_pos/posibility_neg);
        }
        confidencemap.data[ x_offset * confidencemap.M + y_offset] =  1/(1 + exp(-h_patch));
    }
   delete [] selectorInt;
