...
#include "stdafx.h"
#include "cppmatrix.h"
#include "milclassifier_special.h"
                  ...
                  int nrhs, const mxArray *prhs[] )
{
    ...
    haar_ptr = A_IN;
    selector_ptr = B_IN;
    matrix<double> sumimagedata = matrix<double>(C_IN);
    matrix<double> patches = matrix<double>(D_IN);
    ...
    for( int col = 0; col < confidencemap.N; col ++)
        for(int row = 0; row < confidencemap.M; row ++)
            confidencemap.data[index ++] = 0;
            
    milspecialclassify(&patches, &selector, &haarfeature, &sumimagedata, confidencemap);
