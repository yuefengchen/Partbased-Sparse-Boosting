...
for imgno = parameter.imgstart+1:parameter.imgend
    ...
    objregion = meanshift(parameter.patch, confidencemap, parameter.imagewidth, parameter.imageheight);
    
    %[maxx, yind] = max(confidencemap, [], 1);
    %[maxv, xind] = max(maxx);
    parameter.patch = objregion;
    imshow(I);
    rectangle('Position',parameter.patch, 'edgecolor', 'g');
    text(objregion(1) + parameter.patch(3)/2, objregion(2) + parameter.patch(4)/2, num2str(max(max(confidencemap)), '%6f'));
    objectlocation = [objectlocation;parameter.patch];
    confidence = [confidence ; confidencemap(objregion(2), objregion(1))];
